...
def connectFig(conXY,xlon,ylat,legend,borderShape,folder,pol,aveTime):
    fig, ax = plt.subplots()
    cm = 1/2.54  # centimeters in inches
    fig.set_size_inches(15*cm, 10*cm)
    #cmap = plt.get_cmap(cmap, 6)
    x= np.vstack([np.array(conXY)[:,0],np.array(conXY)[:,2]])
    y= np.vstack([np.array(conXY)[:,1],np.array(conXY)[:,3]])
